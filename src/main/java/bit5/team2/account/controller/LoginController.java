...
public class LoginController extends BaseController {
    ...
    public @ResponseBody
    ResultEntity<Object> loginAdmin(@RequestBody @Valid InLogin input, BindingResult bindingResult){
        ResultEntity<Object> errorInput = this.validateInput(bindingResult);
        if (errorInput == null) {
            OutLoginAdmin output = loginService.loginAdmin(input.getUsername(),input.getPassword());
            if (output == null) {
                return this.failed();
            } else {
                return this.success(output);
